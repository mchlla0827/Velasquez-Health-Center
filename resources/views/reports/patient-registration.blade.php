...
@section('content')
<div class="page-title">Patient Registration Report</div>
<p style="color:#6B7280; margin-bottom:16px; font-size:13px;">List of registered patients with registration date and activity status.</p>
...
<form method="GET" class="report-controls">
    ...
    <div style="display:flex; gap:8px;">
        ...
        <a href="{{ route('reports.export.patient-registration', request()->query()) }}" class="btn-outline" style="text-decoration:none; display:inline-block;">Export Excel</a>
    </div>
</form>
...
<div class="content-card" id="printableArea">
    ...
    <div class="print-only" style="display:none; text-align:center; margin-bottom:20px;">
        ...
        <p style="margin:4px 0; font-size:13px;">Patient Registration Report</p>
        <p style="margin:0; font-size:12px; color:#6B7280;">Generated: {{ date('F d, Y | h:i A') }}</p>
    </div>

    <div class="summary-grid" style="grid-template-columns:repeat(1,1fr); max-width:260px;">
        <div class="summary-box"><span>Registration Totals (selected period)</span><h3>{{ $registrationTotal ?? '--' }}</h3></div>
    </div>
...
function exportToExcel() {
    ...
    const header = 'Velasquez Health Center - Patient Registration Report\nGenerated: {{ date("F d, Y | h:i A") }}\n\n';
    const content = header + rows.join('\n');
    const blob = new Blob([content], { type: 'application/vnd.ms-excel;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'patient-registration-{{ date("Y-m-d") }}.xls';

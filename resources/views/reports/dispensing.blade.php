...
@section('content')
<div class="page-title">Medicine Dispensing Report</div>
<p style="color:#6B7280; margin-bottom:16px; font-size:13px;">All medicine dispensing transactions for the selected period.</p>
...
<div class="content-card" id="printableArea">
    ...
    <div class="print-only" style="display:none; text-align:center; margin-bottom:20px;">
        ...
        <p style="margin:4px 0; font-size:13px;">Medicine Dispensing Report</p>
        <p style="margin:0; font-size:12px; color:#6B7280;">Generated: {{ date('F d, Y | h:i A') }}</p>
    </div>

    <div class="summary-grid">
        <div class="summary-box"><span>Total Transactions</span><h3>{{ $totalTransactions ?? '--' }}</h3></div>
        <div class="summary-box"><span>Total Quantity Dispensed</span><h3>{{ $totalDispensed ?? '--' }}</h3></div>
...
function exportToExcel() {
    ...
    const header = 'Velasquez Health Center - Medicine Dispensing Report\nGenerated: {{ date("F d, Y | h:i A") }}\n\n';
    const content = header + rows.join('\n');
    const blob = new Blob([content], { type: 'application/vnd.ms-excel;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'dispensing-report-{{ date("Y-m-d") }}.xls';

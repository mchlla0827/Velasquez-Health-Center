...
@section('content')
<div class="page-title">Daily Service Report</div>
<p style="color:#6B7280; margin-bottom:16px; font-size:13px;">Health center activity broken down by day. Referrals are not yet tracked in this system.</p>
...
<form method="GET" class="report-controls">
    <div class="filters">
        <div class="filter-group">
            ...
            <select name="range" onchange="document.getElementById('customDateWrap').style.display = (this.value === 'custom') ? 'contents' : 'none';">
                <option value="all" {{ ($selectedRange ?? 'all') == 'all' ? 'selected' : '' }}>Last 30 Days (default)</option>
                ...
                <option value="custom" {{ ($selectedRange ?? '') == 'custom' ? 'selected' : '' }}>Custom Range / Specific Day</option>
            </select>
        </div>
    ...
    <div style="display:flex; gap:8px;">
        ...
        <a href="{{ route('reports.export.daily-service', request()->query()) }}" class="btn-outline" style="text-decoration:none; display:inline-block;">Export Excel</a>
    </div>
</form>
...
<div class="content-card" id="printableArea">
    ...
    <div class="print-only" style="display:none; text-align:center; margin-bottom:20px;">
        ...
        <p style="margin:4px 0; font-size:13px;">Daily Service Report</p>
        <p style="margin:0; font-size:12px; color:#6B7280;">Generated: {{ date('F d, Y | h:i A') }}</p>
    </div>

    <div class="summary-grid">
        <div class="summary-box"><span>Total Registrations</span><h3>{{ $summary['total_registrations'] ?? '--' }}</h3></div>
        <div class="summary-box"><span>Total Visits</span><h3>{{ $summary['total_visits'] ?? '--' }}</h3></div>
        <div class="summary-box"><span>Total Dispensing</span><h3>{{ $summary['total_dispensing'] ?? '--' }}</h3></div>
        <div class="summary-box"><span>Total Medicine Requests</span><h3>{{ $summary['total_requests'] ?? '--' }}</h3></div>
    </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Triage Assessments</th>
                    <th>Medicine Dispensing</th>
...
function exportToExcel() {
    ...
    const header = 'Velasquez Health Center - Daily Service Report\nGenerated: {{ date("F d, Y | h:i A") }}\n\n';
    const content = header + rows.join('\n');
    const blob = new Blob([content], { type: 'application/vnd.ms-excel;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = 'daily-service-report-{{ date("Y-m-d") }}.xls';

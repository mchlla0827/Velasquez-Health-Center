...
<head>
    ...
    <style>
        body { margin: 0; font-family: Arial, sans-serif; background: #F9FAFB; color: #1F2937; overflow-x: hidden; }
        .container { display: flex; }
        .main { margin-left: 250px; width: calc(100% - 250px); padding: 24px; box-sizing: border-box; }
        ...
        .role { background: {{ $roleColor ?? '#9333EA' }}; color: white; padding: 2px 10px; border-radius: 9999px; font-size: 11px; font-weight: 600; margin-left: 8px; text-transform: uppercase; }
        ...
        .btn-primary { background: #2563EB; color: white; border: none; padding: 9px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: background 0.2s ease; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; }
        .btn-primary:hover { background: #1D4ED8; }
        .btn-secondary { background: #FFFFFF; color: #374151; border: 1px solid #D1D5DB; padding: 9px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: background 0.2s ease; }
        .btn-secondary:hover { background: #F3F4F6; }

        .stats-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 24px; }
        .stat-card { background: #FFFFFF; padding: 20px 24px; border-radius: 14px; border: 1px solid #E5E7EB; display: flex; flex-direction: column; gap: 6px; transition: transform 0.2s ease; }
        .stat-card:hover { transform: translateY(-2px); }
        .stat-title { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; }
        .stat-number { font-size: 28px; font-weight: 700; line-height: 1; }
        .stat-unit { font-size: 14px; font-weight: 500; }
        .stat-grey { background: #F9FAFB; border-color: #E5E7EB; } .stat-grey .stat-title { color: #6B7280; } .stat-grey .stat-number { color: #111827; }
        .stat-yellow { background: #FFFBEB; border-color: #FDE047; } .stat-yellow .stat-title { color: #D97706; } .stat-yellow .stat-number { color: #92400E; }
        .stat-red { background: #FEF2F2; border-color: #FCA5A5; } .stat-red .stat-title { color: #DC2626; } .stat-red .stat-number { color: #991B1B; }

        .table-card { background: #FFFFFF; border: 1px solid #E5E7EB; border-radius: 14px; overflow: hidden; margin-bottom: 24px; }
        .toolbar-section { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid #F3F4F6; }
        .toolbar-title { font-size: 15px; font-weight: 700; color: #1F2937; }
        .search-input { width: 250px; padding: 8px 12px; font-size: 13px; border: 1px solid #D1D5DB; border-radius: 8px; outline: none; }
        .search-input:focus { border-color: #3B82F6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1); }
        .overflow-x-auto { overflow-x: auto; width: 100%; }
        table { width: 100%; border-collapse: collapse; font-size: 13px; text-align: left; }
        thead th { background: #F9FAFB; color: #4B5563; font-size: 11px; font-weight: 700; letter-spacing: 0.05em; padding: 12px 20px; border-bottom: 1px solid #E5E7EB; text-transform: uppercase; }
        ...
        .text-action { color: #2563EB; font-weight: 600; cursor: pointer; text-decoration: none; }
        .text-action:hover { text-decoration: underline; }
        .text-empty { padding: 40px 20px; text-align: center; color: #6B7280; }

        .analytics-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px; }
        .column-box { display: flex; flex-direction: column; gap: 16px; }
        .info-banner { position: relative; background: #FFFFFF; border: 1px solid #E2E8F0; border-radius: 14px; padding: 20px 24px; }
        ...
        .banner-badge { position: absolute; top: 20px; right: 20px; border: 1px solid; padding: 3px 10px; border-radius: 9999px; font-size: 10px; font-weight: 700; text-transform: uppercase; }
        ...
        .summary-card { background: #FFFFFF; border-radius: 12px; padding: 16px 20px; border: 1px solid #E5E7EB; display: flex; justify-content: space-between; align-items: center; }
        ...
        .trend-container { background: #FFFFFF; border: 1px solid #E5E7EB; border-radius: 14px; padding: 20px; display: flex; flex-direction: column; gap: 16px; }
        .chart-box { height: 180px; width: 100%; }
        .trend-list { display: flex; flex-direction: column; gap: 8px; }
        .trend-item { display: flex; justify-content: space-between; padding: 8px 12px; background: #F9FAFB; border-radius: 8px; font-size: 12px; }
        ...
        .modal-card { background: white; border-radius: 14px; padding: 24px; width: 100%; max-width: 480px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); }
        .modal-content { background: white; border-radius: 14px; padding: 24px; width: calc(100% - 32px); max-width: 480px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); box-sizing: border-box; }
                ...
                <table>
                    ...
                    <tbody id="stockoutTableBody">
                        @forelse($stockOutRecords ?? [] as $record)
                            <tr>
                                <td>{{ \Carbon\Carbon::parse($record->created_at ?? now())->format('M d, Y') }}</td>
                                <td class="font-semibold">{{ $record->medicine->name ?? 'N/A' }}</td>
                                ...
                                <td>
                                    ...
                                    class="text-action" 
                                    data-medicine-name="{{ $record->medicine->name ?? 'N/A' }}"
                                    data-quantity="{{ $record->quantity ?? 0 }}"
                                    data-patients="{{ $record->patients_affected ?? 0 }}"
                                    data-duration="{{ $record->duration ?? 1 }}"
...
<div class="modal-overlay" id="stockOutModal">
    <div class="modal-card">
        ...
        <form action="{{ route($role . '.stockout.store') }}" method="POST">
            @csrf
...
<div id="viewStockoutModal" class="modal-overlay" style="display: none;">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Stock-Out Incident Details</h3>
            <button type="button" class="close-btn" onclick="closeViewModal()" aria-label="Close details">&times;</button>
        </div>
        
        <div class="form-group">
            <label>Medicine Name</label>
            <input type="text" class="form-control" id="view_medicine_name" readonly disabled>
        </div>
        <div class="form-group">
            <label>Quantity Short / Needed</label>
            <input type="number" class="form-control" id="view_quantity_needed" readonly disabled>
        </div>
        <div class="form-group">
            <label>Patients Affected</label>
            <input type="number" class="form-control" id="view_patients_affected" readonly disabled>
        </div>
        <div class="form-group">
            <label>Estimated Stock-Out Duration (Days)</label>
            <input type="number" class="form-control" id="view_duration" readonly disabled>
        </div>
...
<script>
    ...
    if (trendCanvas) {
        ...
        const labels = (Array.isArray(rawTrend) && rawTrend.length) 
            ? rawTrend.map(i => i.month || '') 
            : ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
            
        const dataValues = (Array.isArray(rawTrend) && rawTrend.length) 
            ? rawTrend.map(i => i.count || 0) 
            : [0, 0, 0, 0, 0, 0];
    ...
    // Auto open modal if there are form validation errors upon submit redirect
    @if ($errors->any())
        openModal();
    @endif
    ...
    // Extract data attributes from clicked link
    const medicine = element.getAttribute('data-medicine-name');
    const quantity = element.getAttribute('data-quantity');
    const patients = element.getAttribute('data-patients');
    const duration = element.getAttribute('data-duration');

    // Populate read-only form inputs
    document.getElementById('view_medicine_name').value = medicine;
    document.getElementById('view_quantity_needed').value = quantity;
    document.getElementById('view_patients_affected').value = patients;
    document.getElementById('view_duration').value = duration;

    // Show modal
    document.getElementById('viewStockoutModal').style.display = 'flex';
}
